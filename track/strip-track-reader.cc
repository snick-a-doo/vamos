...
{
    Vamos_Media::XML_Tag::Attribute_List const& attribs = tag.get_attributes();
    ...
    else if (label() == "material")
    {
        ...
        {
            std::cerr << "Strip_Track_Reader: Warning: Unknown material \"" << attribs[1].value
                      << '\"' << std::endl;
...
void Strip_Track_Reader::on_data(std::string data)
{
    if (data.empty())
        return;
    ...
    else if (path() == "/track/start-direction")
    {
        ...
        is >> m_angle;
    else if (path() == "/track/road/pit-in/side" || path() == "/track/road/pit-out/side")
    {
        std::string side;
        is >> side;
